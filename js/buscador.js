...
(function () {
  ...
  const INDICE_KEY = 'guia_indice_cache';
  ...
  const NIVELES = [
    { id: 0,  titulo: 'Antes de empezar' },
    { id: 1,  titulo: 'Lo más básico de Python' },
    { id: 2,  titulo: 'Tipos de datos' },
    { id: 3,  titulo: 'Operaciones' },
    { id: 4,  titulo: 'Control de flujo' },
    { id: 5,  titulo: 'Colecciones' },
    { id: 6,  titulo: 'Funciones' },
    { id: 7,  titulo: 'Módulos y librerías' },
    { id: 8,  titulo: 'Errores y excepciones' },
    { id: 9,  titulo: 'Clases y objetos (POO)' },
    { id: 10, titulo: 'Análisis numérico con Python' },
  ...
  async function construirIndice() {
    ...
    for (const nivel of NIVELES) {
      const file = `contenido/nivel-${String(nivel.id).padStart(2, '0')}.json`;
      try {
        ...
        data.niveles.forEach(n => {
          (n.comandos || []).forEach((cmd, i) => {
            ...
            const textoInter = stripHtml(cmd.intermedio);
            const textoAvanz = stripHtml(cmd.avanzado);
        ...
        console.warn('[buscador] No se pudo cargar nivel-' + nivel.id + '.json:', e);
      }
    }
  ...
  window.reconstruirIndiceBuscador = () => {
